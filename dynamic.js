...
if (mapContainer && typeof jsVectorMap !== 'undefined') {
 new jsVectorMap({
  ...
  // Highlight countries with your research presence
  regions: [
   {
    country: 'GB',
    style: {
  ...
  onRegionTooltipShow: function(event, tooltip) {
   const country = tooltip.name;
   if (country === 'United Kingdom') {
    tooltip.text('🇬🇧 UK: PhD (2019-2024), Research Fellow (2024-Present), 5+ papers');
   } else if (country === 'India') {
    tooltip.text('🇮🇳 India: MTech, BE, Lecturer, 3 code-mixed NLP papers');
   } else if (country === 'United States') {
    tooltip.text('🇺🇸 USA: Citations from top universities');
   } else if (country === 'Germany') {
    tooltip.text('🇩🇪 Germany: ACM SAC, iiWAS venues');
   } else if (country === 'Netherlands') {
    tooltip.text('🇳🇱 Netherlands: iiWAS 2021 (Best Paper)');
   } else if (country === 'Australia') {
    tooltip.text('🇦🇺 Australia: Research collaborations');
   }
  }
 });
 
 console.log('✅ Global Impact Map loaded successfully!');
} else {
 if (!mapContainer) {
  console.warn('⚠️ Geo map container not found');
 }
 if (typeof jsVectorMap === 'undefined') {
  console.warn('⚠️ jsVectorMap library not loaded - check internet connection');
 }
}